...
BEGIN
	...
	*/
	if (p_in = 0) then
		select * from Bets where Bets.Finish = p_finish and not exists (
			select * from User_has_Bets 
			where idBets = Bets_idBets And User_Name = p_UserName );
	else 
		select * from Bets where Finish = p_finish And exists (
			select * from User_has_Bets 
			where idBets = Bets_idBets And User_Name = p_UserName);
	end if;
	#drop table tmptable;
...
BEGIN
	...
	Select Confirm into s1 from Bets where idBets = p_idBets;
	if (s1 = 0) then
		update Bets SET Finish = 2 where idBets = p_idBets;
	end if;
END$$
...
BEGIN
	...
	if (tmp != p_Vote) then
		...
		Select SumA into s1 from bb;
		Select SumB into s5 from bb;
		...
		if (s1+s5 = s2 + s3) then
			if (s2 > s3) then 
				Set s4 = 1; 
				if (s1 = 0) then
					update Bets Set Finish = 2, Confirm = s1, PenaltyParty = s4 where idBets = p_idBets;
				else 
					update Bets Set Finish = 1, Confirm = s1, PenaltyParty = s4 where idBets = p_idBets;
				end if;
			end if;
			if (s2 < s3) then 
				set s4 = 2; 
				if (s5 = 0) then
					update Bets Set Finish = 2, Confirm = s5, PenaltyParty = s4 where idBets = p_idBets;
				else
					update Bets Set Finish = 1, Confirm = s5, PenaltyParty = s4 where idBets = p_idBets;
				end if;
			end if;
			if (s2 = s3) then set s4 = 0; 
				update Bets Set Finish = 2, Confirm = s5, PenaltyParty = s4 where idBets = p_idBets;
			end if;
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `userJoinBet`(
IN BName varchar(45),
IN idB Int,
IN pa Int,
IN UC int,
...
BEGIN
	DECLARE tmp1 INT;
    DECLARE tmp2 INT;
	Insert into User_has_Bets value(BName, idB, pa, UC, VP);
	update Bets SET Sum = Sum + 1 where idBets = idB;

	if (pa = 1) then
		update Bets SET SumA = SumA + 1 where idBets = idB;
	else if (pa = 2) then
		update Bets SET SumB = SumB + 1 where idBets = idB;
	end if;
	end if;
	/*Create temporary table tmptable as 
		Select * From
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `ConfirmQueryBets`(
IN p_UserName VARCHAR(45),
IN p_finish int,
IN p_in int
